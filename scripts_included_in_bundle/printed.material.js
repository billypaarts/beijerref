...
cision.websolution.printedMaterial = !cision.websolution.settings.printedMaterial ? {} : function ($) {
    ...
    function submitPrintedMaterial(recaptchaToken) {
        var name = $('#name').val(),
            company = $('#company').val(),
            address = $('#address').val(),
            code = $('#code').val(),
        ...
        if (!name || !company || !address || !code || !city || !country) {
            ...
            grecaptcha.reset(recaptchaElementPrintedMaterial);
        } else {
            var data = {
                "Name": name,
                "CompanyName": company,
    ...
    return {
        ...
        submitPrintedMaterial: submitPrintedMaterial
    };
}(jQuery);
