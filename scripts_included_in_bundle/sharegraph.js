...
cision.websolution.sharegraph = !cision.websolution.settings.sharegraph ? {} : function ($) {
    var settings = $.extend({}, window.cision.websolution.settings.general),
        ...
        showUnadjustedQuotes = settings.showUnadjustedQuotes,
    ...
    function init(options) {
        ...
        if (settings.showUnadjustedQuotes) {
            $('#share-options-select .chartComparisonType.unadjusted').addClass("selected");
        }
    }
    ...
    function setupEvents() {
        ...
        $("#excel-download, .excel-download").on("click", function () {
            var listOfTickers = "";
            var mainInstrument = $(this).data("symbol") || settings.mainInstruments[0].symbol;
            ...
            window.open(
                settings.serviceEndpoint + "ShareHistory/" + window.cision.websolution.settings.sharegraph.shareHistoryKey + "/csv?indexSymbols=" + listOfTickers + "&mainInstrument=" + mainInstrument + "&startDate=" + startDate + "&endDate=" + endDate,
                "exeldownload",
                ""
            );
        });
    ...
    function setupCustomActions() {
        ...
        actions["unadjusted"] = function (evt, obj) {
            showUnadjustedQuotes = $(obj).hasClass('selected');
            showEndOfDay();
        };
    ...
    function showEndOfDay() {
        settings.typeOfChart = showUnadjustedQuotes ? "EndOfDayUnadjusted" : "EndOfDay";
